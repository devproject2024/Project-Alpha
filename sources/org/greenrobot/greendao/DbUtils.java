...
public class DbUtils {
    ...
    public static int executeSqlScript(Context context, Database database, String str, boolean z) throws IOException {
        int i2;
        String[] split = new String(readAsset(context, str), AppConstants.UTF_8).split(";(\\s)*[\n\r]");
        if (z) {
            i2 = executeSqlStatementsInTx(database, split);
        } else {
            i2 = executeSqlStatements(database, split);
        }
        DaoLog.i("Executed " + i2 + " statements from SQL script '" + str + "'");
        return i2;
    }
