...
final class PaymentRepository$makeUpiTransactionStatusRequest$request$2 implements Response.ErrorListener {
    final /* synthetic */ PaymentMethodDataSource.Callback $callback;

    PaymentRepository$makeUpiTransactionStatusRequest$request$2(PaymentMethodDataSource.Callback callback) {
        this.$callback = callback;
    }

    public final void onErrorResponse(VolleyError volleyError) {
        if (SDKUtility.isAppInvokeFlow()) {
            SDKUtility.sendGaEvents(SDKUtility.getAllInOneEventParams(SDKConstants.AI_API_ERROR_RESPONSE, "UPI Transaction Status"));
