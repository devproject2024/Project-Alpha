package net.one97.paytm.feed.b;

import android.util.SparseIntArray;
import android.view.View;
import android.widget.ImageView;
import androidx.constraintlayout.widget.ConstraintLayout;
import androidx.databinding.ViewDataBinding;
import androidx.databinding.a.d;
import androidx.databinding.e;
import net.one97.paytm.feed.d.a.a;
import net.one97.paytm.feed.repository.models.generic.FeedGeneric;
import net.one97.paytm.feed.repository.models.generic.FeedGenericData;
import net.one97.paytm.feed.repository.models.nestedfeed.NestedFeed;

public final class bl extends bk implements a.C0556a {

    /* renamed from: g  reason: collision with root package name */
    private static final ViewDataBinding.b f33963g = null;

    /* renamed from: h  reason: collision with root package name */
    private static final SparseIntArray f33964h = null;

    /* renamed from: i  reason: collision with root package name */
    private final ConstraintLayout f33965i;
    private final ImageView j;
    private final View.OnClickListener k;
    private long l;

    public final boolean onFieldChange(int i2, Object obj, int i3) {
        return false;
    }

    public bl(e eVar, View view) {
        this(eVar, view, mapBindings(eVar, view, 5, f33963g, f33964h));
    }

    private bl(e eVar, View view, Object[] objArr) {
        super(eVar, view, objArr[4], objArr[1], objArr[3]);
        this.l = -1;
        this.f33965i = objArr[0];
        this.f33965i.setTag((Object) null);
        this.j = objArr[2];
        this.j.setTag((Object) null);
        this.f33957a.setTag((Object) null);
        this.f33958b.setTag((Object) null);
        this.f33959c.setTag((Object) null);
        setRootTag(view);
        this.k = new a(this, 1);
        invalidateAll();
    }

    public final void invalidateAll() {
        synchronized (this) {
            this.l = 8;
        }
        requestRebind();
    }

    public final boolean hasPendingBindings() {
        synchronized (this) {
            if (this.l != 0) {
                return true;
            }
            return false;
        }
    }

    public final boolean setVariable(int i2, Object obj) {
        if (net.one97.paytm.feed.a.f33775b == i2) {
            this.f33961e = (net.one97.paytm.feed.ui.feed.m.e) obj;
            synchronized (this) {
                this.l |= 1;
            }
            notifyPropertyChanged(net.one97.paytm.feed.a.f33775b);
            super.requestRebind();
        } else if (net.one97.paytm.feed.a.f33776c == i2) {
            this.f33962f = (NestedFeed) obj;
            synchronized (this) {
                this.l |= 2;
            }
            notifyPropertyChanged(net.one97.paytm.feed.a.f33776c);
            super.requestRebind();
        } else if (net.one97.paytm.feed.a.f33780g != i2) {
            return false;
        } else {
            this.f33960d = (FeedGeneric) obj;
            synchronized (this) {
                this.l |= 4;
            }
            notifyPropertyChanged(net.one97.paytm.feed.a.f33780g);
            super.requestRebind();
        }
        return true;
    }

    public final void executeBindings() {
        long j2;
        String str;
        String str2;
        String str3;
        String str4;
        synchronized (this) {
            j2 = this.l;
            this.l = 0;
        }
        FeedGeneric feedGeneric = this.f33960d;
        int i2 = 0;
        int i3 = ((j2 & 12) > 0 ? 1 : ((j2 & 12) == 0 ? 0 : -1));
        if (i3 != 0) {
            FeedGenericData feedGenericData = feedGeneric != null ? feedGeneric.getFeedGenericData() : null;
            if (feedGenericData != null) {
                str2 = feedGenericData.getImageUrl();
                str4 = feedGenericData.getStreamUrl();
                str = feedGenericData.getDescription();
                str3 = feedGenericData.getTitle();
            } else {
                str3 = null;
                str2 = null;
                str4 = null;
                str = null;
            }
            boolean z = str4 == null;
            if (i3 != 0) {
                j2 |= z ? 32 : 16;
            }
            if (z) {
                i2 = 8;
            }
        } else {
            str3 = null;
            str2 = null;
            str = null;
        }
        if ((8 & j2) != 0) {
            this.f33965i.setOnClickListener(this.k);
        }
        if ((j2 & 12) != 0) {
            this.j.setVisibility(i2);
            d.a(this.f33957a, (CharSequence) str);
            net.one97.paytm.feed.utility.a.a(this.f33958b, str2, (String) null, (String) null);
            d.a(this.f33959c, (CharSequence) str3);
        }
    }

    public final void a(int i2, View view) {
        net.one97.paytm.feed.ui.feed.m.e eVar = this.f33961e;
        NestedFeed nestedFeed = this.f33962f;
        FeedGeneric feedGeneric = this.f33960d;
        if (eVar != null) {
            net.one97.paytm.feed.ui.feed.m.e.a(getRoot().getContext(), feedGeneric, nestedFeed);
        }
    }
}
