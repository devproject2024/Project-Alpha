...
public class CropOverlayView extends View {
    ...
    public boolean setMultiTouchEnabled(boolean z) {
        if (Build.VERSION.SDK_INT < 11 || this.mMultiTouchEnabled == z) {
            return false;
        }
    ...
    public void setMinCropResultSize(int i2, int i3) {
        this.mCropWindowHandler.setMinCropResultSize(i2, i3);
    }

    public void setMaxCropResultSize(int i2, int i3) {
        this.mCropWindowHandler.setMaxCropResultSize(i2, i3);
    }
    ...
    private void initCropWindow() {
        ...
        float min2 = Math.min(BitmapUtils.getRectRight(this.mBoundsPoints), (float) getWidth());
        if (min > max && min2 > max2) {
            ...
            if (this.mInitialCropWindowRect.width() > 0 && this.mInitialCropWindowRect.height() > 0) {
                ...
                rectF.bottom = rectF.top + (((float) this.mInitialCropWindowRect.width()) / this.mCropWindowHandler.getScaleFactorWidth());
    ...
    private void fixCropWindowRectByRules(RectF rectF) {
        ...
        if (rectF.height() < this.mCropWindowHandler.getMinCropHeight()) {
            float minCropWidth2 = (this.mCropWindowHandler.getMinCropWidth() - rectF.width()) / 2.0f;
            rectF.top -= minCropWidth2;
            rectF.bottom += minCropWidth2;
        }
        ...
        if (rectF.height() > this.mCropWindowHandler.getMaxCropHeight()) {
            float width2 = (rectF.width() - this.mCropWindowHandler.getMaxCropWidth()) / 2.0f;
            rectF.top += width2;
            rectF.bottom -= width2;
        }
        ...
        if (this.mCalcBounds.width() > 0.0f && this.mCalcBounds.height() > 0.0f) {
            ...
            float min2 = Math.min(this.mCalcBounds.right, (float) getWidth());
            if (rectF.left < max) {
                rectF.left = max;
            }
            if (rectF.top < max2) {
        ...
        if (this.mFixAspectRatio && ((double) Math.abs(rectF.width() - (rectF.height() * this.mTargetAspectRatio))) > 0.1d) {
            ...
            float abs2 = Math.abs((rectF.height() * this.mTargetAspectRatio) - rectF.width()) / 2.0f;
            rectF.top += abs2;
            rectF.bottom -= abs2;
        }
    }
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (this.mCropShape != CropImageView.CropShape.RECTANGLE) {
            ...
            float f2 = max;
            float f3 = min;
            canvas2.drawRect(f2, max2, f3, rect.top, this.mBackgroundPaint);
            canvas2.drawRect(f2, rect.bottom, f3, min2, this.mBackgroundPaint);
            canvas2.drawRect(f2, rect.top, rect.left, rect.bottom, this.mBackgroundPaint);
    ...
    private void drawGuidelines(Canvas canvas) {
        if (this.mGuidelinePaint != null) {
            ...
            if (this.mCropShape == CropImageView.CropShape.OVAL) {
                ...
                canvas.drawLine(f3, (rect.top + height2) - sin, f3, (rect.bottom - height2) + sin, this.mGuidelinePaint);
                ...
                canvas.drawLine((rect.left + width2) - cos, f5, (rect.right - width2) + cos, f5, this.mGuidelinePaint);
                return;
            }
            float f6 = rect.left + width;
            float f7 = rect.right - width;
            Canvas canvas2 = canvas;
            canvas2.drawLine(f6, rect.top, f6, rect.bottom, this.mGuidelinePaint);
            canvas.drawLine(f7, rect.top, f7, rect.bottom, this.mGuidelinePaint);
            float f8 = rect.top + height;
            float f9 = rect.bottom - height;
            canvas2.drawLine(rect.left, f8, rect.right, f8, this.mGuidelinePaint);
    ...
    private void drawCorners(Canvas canvas) {
        if (this.mBorderCornerPaint != null) {
            ...
            float f6 = f3 + f5;
            Canvas canvas2 = canvas;
            canvas2.drawLine(rect.left - f5, rect.top - f6, rect.left - f5, rect.top + this.mBorderCornerLength, this.mBorderCornerPaint);
            canvas2.drawLine(rect.left - f6, rect.top - f5, rect.left + this.mBorderCornerLength, rect.top - f5, this.mBorderCornerPaint);
            canvas2.drawLine(rect.right + f5, rect.top - f6, rect.right + f5, rect.top + this.mBorderCornerLength, this.mBorderCornerPaint);
            canvas2.drawLine(rect.right + f6, rect.top - f5, rect.right - this.mBorderCornerLength, rect.top - f5, this.mBorderCornerPaint);
            canvas2.drawLine(rect.left - f5, rect.bottom + f6, rect.left - f5, rect.bottom - this.mBorderCornerLength, this.mBorderCornerPaint);
            canvas2.drawLine(rect.left - f6, rect.bottom + f5, rect.left + this.mBorderCornerLength, rect.bottom + f5, this.mBorderCornerPaint);
            canvas2.drawLine(rect.right + f5, rect.bottom + f6, rect.right + f5, rect.bottom - this.mBorderCornerLength, this.mBorderCornerPaint);
            canvas2.drawLine(rect.right + f6, rect.bottom + f5, rect.right - this.mBorderCornerLength, rect.bottom + f5, this.mBorderCornerPaint);
        }
    }
    ...
    private void onActionDown(float f2, float f3) {
        this.mMoveHandler = this.mCropWindowHandler.getMoveHandler(f2, f3, this.mTouchRadius, this.mCropShape);
    ...
    private void onActionMove(float f2, float f3) {
        if (this.mMoveHandler != null) {
            ...
            RectF rect = this.mCropWindowHandler.getRect();
            this.mMoveHandler.move(rect, f2, f3, this.mCalcBounds, this.mViewWidth, this.mViewHeight, calculateBounds(rect) ? 0.0f : f4, this.mFixAspectRatio, this.mTargetAspectRatio);
    ...
    private boolean calculateBounds(RectF rectF) {
        ...
        float max = Math.max(rectLeft, f18);
