package com.airbnb.lottie.e.a;

import com.airbnb.lottie.e.a.c;
import i.c;
import i.f;
import java.io.EOFException;
import java.io.IOException;

final class e extends c {

    /* renamed from: g  reason: collision with root package name */
    private static final f f6012g = f.encodeUtf8("'\\");

    /* renamed from: h  reason: collision with root package name */
    private static final f f6013h = f.encodeUtf8("\"\\");

    /* renamed from: i  reason: collision with root package name */
    private static final f f6014i = f.encodeUtf8("{}[]:, \n\t\r\f/\\;#=");
    private static final f j = f.encodeUtf8("\n\r");
    private static final f k = f.encodeUtf8("*/");
    private final i.e l;
    private final c m;
    private int n = 0;
    private long o;
    private int p;
    private String q;

    e(i.e eVar) {
        if (eVar != null) {
            this.l = eVar;
            this.m = eVar.b();
            a(6);
            return;
        }
        throw new NullPointerException("source == null");
    }

    public final void a() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 3) {
            a(1);
            this.f6007d[this.f6004a - 1] = 0;
            this.n = 0;
            return;
        }
        throw new a("Expected BEGIN_ARRAY but was " + f() + " at path " + n());
    }

    public final void b() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 4) {
            this.f6004a--;
            int[] iArr = this.f6007d;
            int i3 = this.f6004a - 1;
            iArr[i3] = iArr[i3] + 1;
            this.n = 0;
            return;
        }
        throw new a("Expected END_ARRAY but was " + f() + " at path " + n());
    }

    public final void c() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 1) {
            a(3);
            this.n = 0;
            return;
        }
        throw new a("Expected BEGIN_OBJECT but was " + f() + " at path " + n());
    }

    public final void d() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 2) {
            this.f6004a--;
            this.f6006c[this.f6004a] = null;
            int[] iArr = this.f6007d;
            int i3 = this.f6004a - 1;
            iArr[i3] = iArr[i3] + 1;
            this.n = 0;
            return;
        }
        throw new a("Expected END_OBJECT but was " + f() + " at path " + n());
    }

    public final boolean e() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        return (i2 == 2 || i2 == 4 || i2 == 18) ? false : true;
    }

    public final c.b f() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        switch (i2) {
            case 1:
                return c.b.BEGIN_OBJECT;
            case 2:
                return c.b.END_OBJECT;
            case 3:
                return c.b.BEGIN_ARRAY;
            case 4:
                return c.b.END_ARRAY;
            case 5:
            case 6:
                return c.b.BOOLEAN;
            case 7:
                return c.b.NULL;
            case 8:
            case 9:
            case 10:
            case 11:
                return c.b.STRING;
            case 12:
            case 13:
            case 14:
            case 15:
                return c.b.NAME;
            case 16:
            case 17:
                return c.b.NUMBER;
            case 18:
                return c.b.END_DOCUMENT;
            default:
                throw new AssertionError();
        }
    }

    private int o() throws IOException {
        int i2 = this.f6005b[this.f6004a - 1];
        if (i2 == 1) {
            this.f6005b[this.f6004a - 1] = 2;
        } else if (i2 == 2) {
            int a2 = a(true);
            this.m.g();
            if (a2 != 44) {
                if (a2 == 59) {
                    t();
                } else if (a2 == 93) {
                    this.n = 4;
                    return 4;
                } else {
                    throw a("Unterminated array");
                }
            }
        } else if (i2 == 3 || i2 == 5) {
            this.f6005b[this.f6004a - 1] = 4;
            if (i2 == 5) {
                int a3 = a(true);
                this.m.g();
                if (a3 != 44) {
                    if (a3 == 59) {
                        t();
                    } else if (a3 == 125) {
                        this.n = 2;
                        return 2;
                    } else {
                        throw a("Unterminated object");
                    }
                }
            }
            int a4 = a(true);
            if (a4 == 34) {
                this.m.g();
                this.n = 13;
                return 13;
            } else if (a4 == 39) {
                this.m.g();
                t();
                this.n = 12;
                return 12;
            } else if (a4 != 125) {
                t();
                if (b((int) (char) a4)) {
                    this.n = 14;
                    return 14;
                }
                throw a("Expected name");
            } else if (i2 != 5) {
                this.m.g();
                this.n = 2;
                return 2;
            } else {
                throw a("Expected name");
            }
        } else if (i2 == 4) {
            this.f6005b[this.f6004a - 1] = 5;
            int a5 = a(true);
            this.m.g();
            if (a5 != 58) {
                if (a5 == 61) {
                    t();
                    if (this.l.b(1) && this.m.c(0) == 62) {
                        this.m.g();
                    }
                } else {
                    throw a("Expected ':'");
                }
            }
        } else if (i2 == 6) {
            this.f6005b[this.f6004a - 1] = 7;
        } else if (i2 == 7) {
            if (a(false) == -1) {
                this.n = 18;
                return 18;
            }
            t();
        } else if (i2 == 8) {
            throw new IllegalStateException("JsonReader is closed");
        }
        int a6 = a(true);
        if (a6 == 34) {
            this.m.g();
            this.n = 9;
            return 9;
        } else if (a6 != 39) {
            if (!(a6 == 44 || a6 == 59)) {
                if (a6 == 91) {
                    this.m.g();
                    this.n = 3;
                    return 3;
                } else if (a6 != 93) {
                    if (a6 != 123) {
                        int p2 = p();
                        if (p2 != 0) {
                            return p2;
                        }
                        int q2 = q();
                        if (q2 != 0) {
                            return q2;
                        }
                        if (b((int) this.m.c(0))) {
                            t();
                            this.n = 10;
                            return 10;
                        }
                        throw a("Expected value");
                    }
                    this.m.g();
                    this.n = 1;
                    return 1;
                } else if (i2 == 1) {
                    this.m.g();
                    this.n = 4;
                    return 4;
                }
            }
            if (i2 == 1 || i2 == 2) {
                t();
                this.n = 7;
                return 7;
            }
            throw a("Unexpected value");
        } else {
            t();
            this.m.g();
            this.n = 8;
            return 8;
        }
    }

    private int p() throws IOException {
        String str;
        String str2;
        int i2;
        byte c2 = this.m.c(0);
        if (c2 == 116 || c2 == 84) {
            i2 = 5;
            str2 = "true";
            str = "TRUE";
        } else if (c2 == 102 || c2 == 70) {
            i2 = 6;
            str2 = "false";
            str = "FALSE";
        } else if (c2 != 110 && c2 != 78) {
            return 0;
        } else {
            i2 = 7;
            str2 = "null";
            str = "NULL";
        }
        int length = str2.length();
        int i3 = 1;
        while (i3 < length) {
            int i4 = i3 + 1;
            if (!this.l.b((long) i4)) {
                return 0;
            }
            byte c3 = this.m.c((long) i3);
            if (c3 != str2.charAt(i3) && c3 != str.charAt(i3)) {
                return 0;
            }
            i3 = i4;
        }
        if (this.l.b((long) (length + 1)) && b((int) this.m.c((long) length))) {
            return 0;
        }
        this.m.i((long) length);
        this.n = i2;
        return i2;
    }

    private int q() throws IOException {
        byte c2;
        long j2 = 0;
        long j3 = 0;
        int i2 = 0;
        char c3 = 0;
        boolean z = false;
        boolean z2 = true;
        while (true) {
            int i3 = i2 + 1;
            if (!this.l.b((long) i3)) {
                break;
            }
            c2 = this.m.c((long) i2);
            if (c2 != 43) {
                if (c2 != 69 && c2 != 101) {
                    if (c2 != 45) {
                        if (c2 != 46) {
                            if (c2 >= 48 && c2 <= 57) {
                                if (c3 == 1 || c3 == 0) {
                                    j3 = (long) (-(c2 - 48));
                                    c3 = 2;
                                } else if (c3 == 2) {
                                    if (j3 == j2) {
                                        return 0;
                                    }
                                    long j4 = (10 * j3) - ((long) (c2 - 48));
                                    int i4 = (j3 > -922337203685477580L ? 1 : (j3 == -922337203685477580L ? 0 : -1));
                                    z2 = (i4 > 0 || (i4 == 0 && j4 < j3)) & z2;
                                    j3 = j4;
                                } else if (c3 == 3) {
                                    c3 = 4;
                                } else if (c3 == 5 || c3 == 6) {
                                    c3 = 7;
                                }
                            }
                        } else if (c3 != 2) {
                            return 0;
                        } else {
                            c3 = 3;
                        }
                    } else if (c3 == 0) {
                        c3 = 1;
                        z = true;
                    } else if (c3 != 5) {
                        return 0;
                    }
                    i2 = i3;
                    j2 = 0;
                } else if (c3 != 2 && c3 != 4) {
                    return 0;
                } else {
                    c3 = 5;
                    i2 = i3;
                    j2 = 0;
                }
            } else if (c3 != 5) {
                return 0;
            }
            c3 = 6;
            i2 = i3;
            j2 = 0;
        }
        if (b((int) c2)) {
            return 0;
        }
        if (c3 == 2 && z2 && ((j3 != Long.MIN_VALUE || z) && (j3 != 0 || !z))) {
            if (!z) {
                j3 = -j3;
            }
            this.o = j3;
            this.m.i((long) i2);
            this.n = 16;
            return 16;
        } else if (c3 != 2 && c3 != 4 && c3 != 7) {
            return 0;
        } else {
            this.p = i2;
            this.n = 17;
            return 17;
        }
    }

    private boolean b(int i2) throws IOException {
        if (i2 == 9 || i2 == 10 || i2 == 12 || i2 == 13 || i2 == 32) {
            return false;
        }
        if (i2 != 35) {
            if (i2 == 44) {
                return false;
            }
            if (!(i2 == 47 || i2 == 61)) {
                if (i2 == 123 || i2 == 125 || i2 == 58) {
                    return false;
                }
                if (i2 != 59) {
                    switch (i2) {
                        case 91:
                        case 93:
                            return false;
                        case 92:
                            break;
                        default:
                            return true;
                    }
                }
            }
        }
        t();
        return false;
    }

    public final String g() throws IOException {
        String str;
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 14) {
            str = r();
        } else if (i2 == 13) {
            str = a(f6013h);
        } else if (i2 == 12) {
            str = a(f6012g);
        } else if (i2 == 15) {
            str = this.q;
        } else {
            throw new a("Expected a name but was " + f() + " at path " + n());
        }
        this.n = 0;
        this.f6006c[this.f6004a - 1] = str;
        return str;
    }

    public final int a(c.a aVar) throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 < 12 || i2 > 15) {
            return -1;
        }
        if (i2 == 15) {
            return a(this.q, aVar);
        }
        int a2 = this.l.a(aVar.f6011b);
        if (a2 != -1) {
            this.n = 0;
            this.f6006c[this.f6004a - 1] = aVar.f6010a[a2];
            return a2;
        }
        String str = this.f6006c[this.f6004a - 1];
        String g2 = g();
        int a3 = a(g2, aVar);
        if (a3 == -1) {
            this.n = 15;
            this.q = g2;
            this.f6006c[this.f6004a - 1] = str;
        }
        return a3;
    }

    public final void h() throws IOException {
        if (!this.f6009f) {
            int i2 = this.n;
            if (i2 == 0) {
                i2 = o();
            }
            if (i2 == 14) {
                s();
            } else if (i2 == 13) {
                b(f6013h);
            } else if (i2 == 12) {
                b(f6012g);
            } else if (i2 != 15) {
                throw new a("Expected a name but was " + f() + " at path " + n());
            }
            this.n = 0;
            this.f6006c[this.f6004a - 1] = "null";
            return;
        }
        throw new a("Cannot skip unexpected " + f() + " at " + n());
    }

    private int a(String str, c.a aVar) {
        int length = aVar.f6010a.length;
        for (int i2 = 0; i2 < length; i2++) {
            if (str.equals(aVar.f6010a[i2])) {
                this.n = 0;
                this.f6006c[this.f6004a - 1] = str;
                return i2;
            }
        }
        return -1;
    }

    public final String i() throws IOException {
        String str;
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 10) {
            str = r();
        } else if (i2 == 9) {
            str = a(f6013h);
        } else if (i2 == 8) {
            str = a(f6012g);
        } else if (i2 == 11) {
            str = this.q;
            this.q = null;
        } else if (i2 == 16) {
            str = Long.toString(this.o);
        } else if (i2 == 17) {
            str = this.m.e((long) this.p);
        } else {
            throw new a("Expected a string but was " + f() + " at path " + n());
        }
        this.n = 0;
        int[] iArr = this.f6007d;
        int i3 = this.f6004a - 1;
        iArr[i3] = iArr[i3] + 1;
        return str;
    }

    public final boolean j() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 5) {
            this.n = 0;
            int[] iArr = this.f6007d;
            int i3 = this.f6004a - 1;
            iArr[i3] = iArr[i3] + 1;
            return true;
        } else if (i2 == 6) {
            this.n = 0;
            int[] iArr2 = this.f6007d;
            int i4 = this.f6004a - 1;
            iArr2[i4] = iArr2[i4] + 1;
            return false;
        } else {
            throw new a("Expected a boolean but was " + f() + " at path " + n());
        }
    }

    public final double k() throws IOException {
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 16) {
            this.n = 0;
            int[] iArr = this.f6007d;
            int i3 = this.f6004a - 1;
            iArr[i3] = iArr[i3] + 1;
            return (double) this.o;
        }
        if (i2 == 17) {
            this.q = this.m.e((long) this.p);
        } else if (i2 == 9) {
            this.q = a(f6013h);
        } else if (i2 == 8) {
            this.q = a(f6012g);
        } else if (i2 == 10) {
            this.q = r();
        } else if (i2 != 11) {
            throw new a("Expected a double but was " + f() + " at path " + n());
        }
        this.n = 11;
        try {
            double parseDouble = Double.parseDouble(this.q);
            if (this.f6008e || (!Double.isNaN(parseDouble) && !Double.isInfinite(parseDouble))) {
                this.q = null;
                this.n = 0;
                int[] iArr2 = this.f6007d;
                int i4 = this.f6004a - 1;
                iArr2[i4] = iArr2[i4] + 1;
                return parseDouble;
            }
            throw new b("JSON forbids NaN and infinities: " + parseDouble + " at path " + n());
        } catch (NumberFormatException unused) {
            throw new a("Expected a double but was " + this.q + " at path " + n());
        }
    }

    private String a(f fVar) throws IOException {
        StringBuilder sb = null;
        while (true) {
            long c2 = this.l.c(fVar);
            if (c2 == -1) {
                throw a("Unterminated string");
            } else if (this.m.c(c2) == 92) {
                if (sb == null) {
                    sb = new StringBuilder();
                }
                sb.append(this.m.e(c2));
                this.m.g();
                sb.append(w());
            } else if (sb == null) {
                String e2 = this.m.e(c2);
                this.m.g();
                return e2;
            } else {
                sb.append(this.m.e(c2));
                this.m.g();
                return sb.toString();
            }
        }
    }

    private String r() throws IOException {
        long c2 = this.l.c(f6014i);
        return c2 != -1 ? this.m.e(c2) : this.m.q();
    }

    private void b(f fVar) throws IOException {
        while (true) {
            long c2 = this.l.c(fVar);
            if (c2 == -1) {
                throw a("Unterminated string");
            } else if (this.m.c(c2) == 92) {
                this.m.i(c2 + 1);
                w();
            } else {
                this.m.i(c2 + 1);
                return;
            }
        }
    }

    private void s() throws IOException {
        long c2 = this.l.c(f6014i);
        i.c cVar = this.m;
        if (c2 == -1) {
            c2 = cVar.f46277b;
        }
        cVar.i(c2);
    }

    public final int l() throws IOException {
        String str;
        int i2 = this.n;
        if (i2 == 0) {
            i2 = o();
        }
        if (i2 == 16) {
            long j2 = this.o;
            int i3 = (int) j2;
            if (j2 == ((long) i3)) {
                this.n = 0;
                int[] iArr = this.f6007d;
                int i4 = this.f6004a - 1;
                iArr[i4] = iArr[i4] + 1;
                return i3;
            }
            throw new a("Expected an int but was " + this.o + " at path " + n());
        }
        if (i2 == 17) {
            this.q = this.m.e((long) this.p);
        } else if (i2 == 9 || i2 == 8) {
            if (i2 == 9) {
                str = a(f6013h);
            } else {
                str = a(f6012g);
            }
            this.q = str;
            try {
                int parseInt = Integer.parseInt(this.q);
                this.n = 0;
                int[] iArr2 = this.f6007d;
                int i5 = this.f6004a - 1;
                iArr2[i5] = iArr2[i5] + 1;
                return parseInt;
            } catch (NumberFormatException unused) {
            }
        } else if (i2 != 11) {
            throw new a("Expected an int but was " + f() + " at path " + n());
        }
        this.n = 11;
        try {
            double parseDouble = Double.parseDouble(this.q);
            int i6 = (int) parseDouble;
            if (((double) i6) == parseDouble) {
                this.q = null;
                this.n = 0;
                int[] iArr3 = this.f6007d;
                int i7 = this.f6004a - 1;
                iArr3[i7] = iArr3[i7] + 1;
                return i6;
            }
            throw new a("Expected an int but was " + this.q + " at path " + n());
        } catch (NumberFormatException unused2) {
            throw new a("Expected an int but was " + this.q + " at path " + n());
        }
    }

    public final void close() throws IOException {
        this.n = 0;
        this.f6005b[0] = 8;
        this.f6004a = 1;
        this.m.t();
        this.l.close();
    }

    public final void m() throws IOException {
        if (!this.f6009f) {
            int i2 = 0;
            do {
                int i3 = this.n;
                if (i3 == 0) {
                    i3 = o();
                }
                if (i3 == 3) {
                    a(1);
                } else if (i3 == 1) {
                    a(3);
                } else {
                    if (i3 == 4) {
                        i2--;
                        if (i2 >= 0) {
                            this.f6004a--;
                        } else {
                            throw new a("Expected a value but was " + f() + " at path " + n());
                        }
                    } else if (i3 == 2) {
                        i2--;
                        if (i2 >= 0) {
                            this.f6004a--;
                        } else {
                            throw new a("Expected a value but was " + f() + " at path " + n());
                        }
                    } else if (i3 == 14 || i3 == 10) {
                        s();
                    } else if (i3 == 9 || i3 == 13) {
                        b(f6013h);
                    } else if (i3 == 8 || i3 == 12) {
                        b(f6012g);
                    } else if (i3 == 17) {
                        this.m.i((long) this.p);
                    } else if (i3 == 18) {
                        throw new a("Expected a value but was " + f() + " at path " + n());
                    }
                    this.n = 0;
                }
                i2++;
                this.n = 0;
            } while (i2 != 0);
            int[] iArr = this.f6007d;
            int i4 = this.f6004a - 1;
            iArr[i4] = iArr[i4] + 1;
            this.f6006c[this.f6004a - 1] = "null";
            return;
        }
        throw new a("Cannot skip unexpected " + f() + " at " + n());
    }

    /* JADX WARNING: Code restructure failed: missing block: B:12:0x0024, code lost:
        r6.m.i((long) (r3 - 1));
     */
    /* JADX WARNING: Code restructure failed: missing block: B:13:0x002e, code lost:
        if (r1 != 47) goto L_0x0073;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:15:0x0038, code lost:
        if (r6.l.b(2) != false) goto L_0x003b;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:16:0x003a, code lost:
        return r1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:17:0x003b, code lost:
        t();
        r3 = r6.m.c(1);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:18:0x0048, code lost:
        if (r3 == 42) goto L_0x005b;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:19:0x004a, code lost:
        if (r3 == 47) goto L_0x004d;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:20:0x004c, code lost:
        return r1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:21:0x004d, code lost:
        r6.m.g();
        r6.m.g();
        u();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:22:0x005b, code lost:
        r6.m.g();
        r6.m.g();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:23:0x0069, code lost:
        if (v() == false) goto L_0x006c;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:25:0x0072, code lost:
        throw a("Unterminated comment");
     */
    /* JADX WARNING: Code restructure failed: missing block: B:27:0x0075, code lost:
        if (r1 != 35) goto L_0x007e;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:28:0x0077, code lost:
        t();
        u();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:29:0x007e, code lost:
        return r1;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private int a(boolean r7) throws java.io.IOException {
        /*
            r6 = this;
            r0 = 0
        L_0x0001:
            r1 = 0
        L_0x0002:
            i.e r2 = r6.l
            int r3 = r1 + 1
            long r4 = (long) r3
            boolean r2 = r2.b((long) r4)
            if (r2 == 0) goto L_0x0081
            i.c r2 = r6.m
            long r4 = (long) r1
            byte r1 = r2.c((long) r4)
            r2 = 10
            if (r1 == r2) goto L_0x007f
            r2 = 32
            if (r1 == r2) goto L_0x007f
            r2 = 13
            if (r1 == r2) goto L_0x007f
            r2 = 9
            if (r1 == r2) goto L_0x007f
            i.c r2 = r6.m
            int r3 = r3 + -1
            long r3 = (long) r3
            r2.i((long) r3)
            r2 = 47
            if (r1 != r2) goto L_0x0073
            i.e r3 = r6.l
            r4 = 2
            boolean r3 = r3.b((long) r4)
            if (r3 != 0) goto L_0x003b
            return r1
        L_0x003b:
            r6.t()
            i.c r3 = r6.m
            r4 = 1
            byte r3 = r3.c((long) r4)
            r4 = 42
            if (r3 == r4) goto L_0x005b
            if (r3 == r2) goto L_0x004d
            return r1
        L_0x004d:
            i.c r1 = r6.m
            r1.g()
            i.c r1 = r6.m
            r1.g()
            r6.u()
            goto L_0x0001
        L_0x005b:
            i.c r1 = r6.m
            r1.g()
            i.c r1 = r6.m
            r1.g()
            boolean r1 = r6.v()
            if (r1 == 0) goto L_0x006c
            goto L_0x0001
        L_0x006c:
            java.lang.String r7 = "Unterminated comment"
            com.airbnb.lottie.e.a.b r7 = r6.a((java.lang.String) r7)
            throw r7
        L_0x0073:
            r2 = 35
            if (r1 != r2) goto L_0x007e
            r6.t()
            r6.u()
            goto L_0x0001
        L_0x007e:
            return r1
        L_0x007f:
            r1 = r3
            goto L_0x0002
        L_0x0081:
            if (r7 != 0) goto L_0x0085
            r7 = -1
            return r7
        L_0x0085:
            java.io.EOFException r7 = new java.io.EOFException
            java.lang.String r0 = "End of input"
            r7.<init>(r0)
            throw r7
        */
        throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.e.a.e.a(boolean):int");
    }

    private void t() throws IOException {
        if (!this.f6008e) {
            throw a("Use JsonReader.setLenient(true) to accept malformed JSON");
        }
    }

    private void u() throws IOException {
        long c2 = this.l.c(j);
        i.c cVar = this.m;
        cVar.i(c2 != -1 ? c2 + 1 : cVar.f46277b);
    }

    private boolean v() throws IOException {
        long b2 = this.l.b(k);
        boolean z = b2 != -1;
        i.c cVar = this.m;
        cVar.i(z ? b2 + ((long) k.size()) : cVar.f46277b);
        return z;
    }

    public final String toString() {
        return "JsonReader(" + this.l + ")";
    }

    private char w() throws IOException {
        int i2;
        int i3;
        if (this.l.b(1)) {
            byte g2 = this.m.g();
            if (g2 == 10 || g2 == 34 || g2 == 39 || g2 == 47 || g2 == 92) {
                return (char) g2;
            }
            if (g2 == 98) {
                return 8;
            }
            if (g2 == 102) {
                return 12;
            }
            if (g2 == 110) {
                return 10;
            }
            if (g2 == 114) {
                return 13;
            }
            if (g2 == 116) {
                return 9;
            }
            if (g2 != 117) {
                if (this.f6008e) {
                    return (char) g2;
                }
                throw a("Invalid escape sequence: \\" + ((char) g2));
            } else if (this.l.b(4)) {
                char c2 = 0;
                for (int i4 = 0; i4 < 4; i4++) {
                    byte c3 = this.m.c((long) i4);
                    char c4 = (char) (c2 << 4);
                    if (c3 < 48 || c3 > 57) {
                        if (c3 >= 97 && c3 <= 102) {
                            i2 = c3 - 97;
                        } else if (c3 < 65 || c3 > 70) {
                            throw a("\\u" + this.m.e(4));
                        } else {
                            i2 = c3 - 65;
                        }
                        i3 = i2 + 10;
                    } else {
                        i3 = c3 - 48;
                    }
                    c2 = (char) (c4 + i3);
                }
                this.m.i(4);
                return c2;
            } else {
                throw new EOFException("Unterminated escape sequence at path " + n());
            }
        } else {
            throw a("Unterminated escape sequence");
        }
    }
}
