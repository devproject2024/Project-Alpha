package com.google.android.youtube.player;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.util.Base64;
import com.google.android.youtube.player.internal.aa;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;

public final class a {

    /* renamed from: a  reason: collision with root package name */
    private static final byte[][] f37545a = {Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    public static b a(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String a2 = aa.a(context);
            return !a(packageManager.getPackageInfo(a2, 64)) ? b.SERVICE_INVALID : aa.a(context, a2) ? b.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(a2, 0).enabled ? b.SERVICE_DISABLED : b.SUCCESS;
        } catch (PackageManager.NameNotFoundException unused) {
            return b.SERVICE_MISSING;
        }
    }

    private static boolean a(PackageInfo packageInfo) {
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            if (packageInfo.signatures.length != 1) {
                return false;
            }
            byte[] byteArray = packageInfo.signatures[0].toByteArray();
            int i2 = 0;
            while (true) {
                byte[][] bArr = f37545a;
                if (i2 >= bArr.length) {
                    break;
                } else if (Arrays.equals(bArr[i2], byteArray)) {
                    return true;
                } else {
                    i2++;
                }
            }
            return false;
        } catch (CertificateException unused) {
        }
    }
}
